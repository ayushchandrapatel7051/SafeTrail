...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        const { trip_name, start_location, destination, trusted_contacts, current_latitude, current_longitude, } = req.body;
        if (!trip_name || !start_location) {
            return res.status(400).json({ error: 'Trip name and start location are required' });
        }
        const shareToken = generateShareToken();
        const result = await query(`INSERT INTO live_trips (
...
router.get('/my-trips', authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        const result = await query(`SELECT * FROM live_trips 
...
router.put('/:id/location', authMiddleware, async (req, res) => {
    try {
        ...
        await query(`UPDATE live_trips 
       SET current_latitude = $1, 
           current_longitude = $2,
           route_waypoints = $3,
           last_updated = NOW()
       WHERE id = $4`, [latitude, longitude, JSON.stringify(route_waypoints || null), id]);
...
// Trigger emergency
router.post('/:id/emergency', authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        const { id } = req.params;
        const { message } = req.body;
        // Verify trip belongs to user
        const checkResult = await query('SELECT id, trusted_contacts FROM live_trips WHERE id = $1 AND user_id = $2', [id, userId]);
        if (checkResult.rows.length === 0) {
            return res.status(404).json({ error: 'Trip not found' });
        }
        await query(`UPDATE live_trips 
       SET emergency_triggered = true,
           emergency_message = $1,
           last_updated = NOW()
       WHERE id = $2`, [message || 'Emergency button triggered', id]);
...
//# sourceMappingURL=liveTrips.js.map