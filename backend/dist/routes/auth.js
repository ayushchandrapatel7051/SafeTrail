...
router.post('/register', async (req, res) => {
    try {
        ...
        const result = await query('INSERT INTO users (email, password_hash, full_name, role, email_verified) VALUES ($1, $2, $3, $4, $5) RETURNING id, email, full_name, role, email_verified', [email, passwordHash, full_name, 'user', false]);
        ...
        await query('INSERT INTO email_verification_otp (user_id, otp, expires_at) VALUES ($1, $2, $3)', [user.id, otp, expiresAt]);
        // Send OTP email
        try {
...
router.post('/verify-otp', async (req, res) => {
    try {
        ...
        // Find OTP record
        const otpResult = await query('SELECT id, otp, expires_at, attempts FROM email_verification_otp WHERE user_id = $1', [user.id]);
...
router.post('/resend-otp', async (req, res) => {
    try {
        ...
        const userResult = await query('SELECT id, email, full_name, email_verified FROM users WHERE email = $1', [email]);
        if (userResult.rows.length === 0) {
            return res.status(400).json({ error: 'User not found' });
        }
        const user = userResult.rows[0];
...
router.post('/login', async (req, res) => {
    try {
        ...
        await query('DELETE FROM email_verification_otp WHERE user_id = $1', [user.id]);
        // Store login OTP
        await query('INSERT INTO email_verification_otp (user_id, otp, expires_at) VALUES ($1, $2, $3)', [user.id, loginOtp, expiresAt]);
        // Send OTP email
        try {
...
router.post('/verify-login-otp', async (req, res) => {
    try {
        ...
        // Find user
        const userResult = await query('SELECT id, email, full_name, role FROM users WHERE email = $1', [email]);
        if (userResult.rows.length === 0) {
