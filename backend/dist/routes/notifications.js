...
router.post('/send', authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        const { title, incident_type, message, recipient_emails, send_to_all_users, send_to_city_users, } = req.body;
        if (!title || !message) {
            return res.status(400).json({ error: 'Title and message are required' });
        }
        // Insert notification record
        const notificationResult = await query(`INSERT INTO admin_notifications (
      ...
      RETURNING *`, [
        ...
        if (send_to_all_users) {
            const usersResult = await query('SELECT email FROM users WHERE email_verified = true AND role = $1', ['user']);
            ...
            const usersResult = await query(`SELECT DISTINCT u.email 
         FROM users u
         JOIN reports r ON u.id = r.user_id
         JOIN places p ON r.place_id = p.id
         WHERE p.city_id = $1 AND u.email_verified = true`, [send_to_city_users]);
            recipientEmails = usersResult.rows.map((row) => row.email);
        }
        ...
        await query(`UPDATE admin_notifications 
       SET status = 'sent', 
           sent_at = NOW(),
           recipient_emails = $1
       WHERE id = $2`, [recipientEmails, notification.id]);
        res.json({
