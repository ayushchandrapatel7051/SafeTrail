...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await query(`SELECT tp.*, c.name as city_name,
...
router.get('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const userId = req.user?.id;
        // Get trip plan
        const planResult = await query(`SELECT tp.*, c.name as city_name
       FROM trip_plans tp
       LEFT JOIN cities c ON tp.city_id = c.id
       WHERE tp.id = $1 AND tp.user_id = $2`, [id, userId]);
        ...
        const itemsResult = await query(`SELECT tpi.*,
       ...
       ORDER BY tpi.day_number, tpi.order_index`, [id]);
...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        // Create trip plan
        const planResult = await query(`INSERT INTO trip_plans (user_id, city_id, name, start_date, end_date, preferences, notes, travelers)
       ...
       RETURNING *`, [
        ...
        if (items && items.length > 0) {
            for (const item of items) {
                await query(`INSERT INTO trip_plan_items 
        ...
        const completeResult = await query(`SELECT tp.*, c.name as city_name
       FROM trip_plans tp
       LEFT JOIN cities c ON tp.city_id = c.id
       WHERE tp.id = $1`, [tripPlan.id]);
...
router.delete('/:id', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        const result = await query('DELETE FROM trip_plans WHERE id = $1 AND user_id = $2 RETURNING id', [id, userId]);
...
//# sourceMappingURL=tripPlans.js.map