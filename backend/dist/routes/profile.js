...
router.get('/me', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await query(`SELECT id, email, full_name, role, email_verified, created_at, updated_at 
...
router.put('/me', authMiddleware, async (req, res) => {
    try {
       ...
       WHERE id = $2 
       RETURNING id, email, full_name, role, email_verified, created_at, updated_at`, [full_name, userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        res.json({
...
router.get('/stats', authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        // Get reports count
        const reportsResult = await query('SELECT COUNT(*) as count FROM reports WHERE user_id = $1', [userId]);
        const reportsCount = parseInt(reportsResult.rows[0].count);
        // Get verified reports count
        const verifiedReportsResult = await query('SELECT COUNT(*) as count FROM reports WHERE user_id = $1 AND status = $2', [userId, 'verified']);
        const verifiedReportsCount = parseInt(verifiedReportsResult.rows[0].count);
        // Get unique cities from user's reports
        const citiesResult = await query(`SELECT COUNT(DISTINCT p.city_id) as count 
       FROM reports r 
       ...
       WHERE r.user_id = $1`, [userId]);
        ...
        const placesResult = await query('SELECT COUNT(DISTINCT place_id) as count FROM reports WHERE user_id = $1', [userId]);
