...
router.post('/', authMiddleware, upload.single('photo'), async (req, res) => {
    try {
        const { place_id, type, description, latitude, longitude, severity, is_anonymous } = req.body;
        // Validate input
        ...
        // Create report
        const result = await query(`INSERT INTO reports (
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const reportResult = await query(`SELECT r.id, r.user_id, r.place_id,
              p.name as place_name,
              c.name as city_name,
              u.full_name as reporter_name,
              u.email as reporter_email,
...
router.patch('/:id/verify', authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        const reportResult = await query('SELECT place_id, user_id, is_anonymous FROM reports WHERE id = $1', [id]);
        if (reportResult.rows.length === 0) {
            return res.status(404).json({ error: 'Report not found' });
        }
