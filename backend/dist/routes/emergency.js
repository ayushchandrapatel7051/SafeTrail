...
router.get('/place/:placeId', async (req, res) => {
    try {
        ...
        const emergencyResult = await query(`SELECT 
        p.id as place_id,
        p.name as place_name,
        p.latitude as place_lat,
        p.longitude as place_lon,
       ...
       WHERE es.place_id = $1`, [placeId]);
        ...
        // Get hospitals
        const hospitalsResult = await query(`SELECT * FROM hospitals WHERE place_id = $1 ORDER BY distance_m`, [placeId]);
        const response = {
            ...emergency,
...
router.get('/contacts', authMiddleware, async (req, res) => {
    try {
        const result = await query('SELECT * FROM emergency_contacts WHERE user_id = $1 ORDER BY is_primary DESC, created_at ASC', [req.user.id]);
        res.json(result.rows);
    }
    catch (error) {
...
router.post('/contacts', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await query(`INSERT INTO emergency_contacts (user_id, name, email, relationship, is_primary)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`, [req.user.id, name, email, relationship, is_primary || false]);
...
router.put('/contacts/:id', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { name, email, relationship, is_primary } = req.body;
        const result = await query(`UPDATE emergency_contacts 
       SET name = $1, email = $2, relationship = $3, is_primary = $4, updated_at = CURRENT_TIMESTAMP
       WHERE id = $5 AND user_id = $6 RETURNING *`, [name, email, relationship, is_primary, id, req.user.id]);
...
router.delete('/contacts/:id', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const result = await query('DELETE FROM emergency_contacts WHERE id = $1 AND user_id = $2 RETURNING *', [id, req.user.id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Contact not found' });
        }
...
router.get('/medical-info', authMiddleware, async (req, res) => {
    try {
...
// Update medical info
router.put('/medical-info', authMiddleware, async (req, res) => {
    try {
...
router.post('/sos', authMiddleware, async (req, res) => {
    try {
        ...
        // Create SOS alert
        const alertResult = await query(`INSERT INTO sos_alerts (user_id, location, message)
       VALUES ($1, $2, $3) RETURNING *`, [req.user.id, JSON.stringify(location), message || 'Emergency! I need immediate help!']);
...
router.get('/sos/history', authMiddleware, async (req, res) => {
    try {
        const result = await query('SELECT * FROM sos_alerts WHERE user_id = $1 ORDER BY created_at DESC LIMIT 20', [req.user.id]);
        res.json(result.rows);
    }
    catch (error) {
