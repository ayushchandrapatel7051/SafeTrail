...
export async function updatePlaceReportCount(placeId) {
    try {
...
export async function updateCityReportCount(cityId) {
    try {
        const result = await query(`SELECT COUNT(*) as count 
       FROM reports r
       JOIN places p ON r.place_id = p.id
       WHERE p.city_id = $1`, [cityId]);
        const count = parseInt(result.rows[0].count);
...
export async function updateCityPlacesCount(cityId) {
    try {
