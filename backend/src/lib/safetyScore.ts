...
export async function calculateSafetyScore(placeId: number): Promise<number> {
  try {
    ...
    const data = result.rows[0] as SafetyScoreData;
...
export async function updatePlaceSafetyScore(placeId: number): Promise<void> {
  try {
    const score = await calculateSafetyScore(placeId);
    await query(
      'UPDATE places SET safety_score = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2',
      [score, placeId]
    );
  } catch (error) {
...
export async function updateCitySafetyScore(cityId: number): Promise<void> {
  try {
    const result = await query(
      `SELECT AVG(safety_score) as avg_score FROM places WHERE city_id = $1`,
      [cityId]
    );

    const avgScore = result.rows[0].avg_score || 50;
    await query(
      'UPDATE cities SET safety_score = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2',
      [Math.round(avgScore * 10) / 10, cityId]
    );
  } catch (error) {
...
export async function getSafetyStatus(score: number): Promise<'safe' | 'caution' | 'danger'> {
...
export async function updateCityReportCount(cityId: number): Promise<void> {
  try {
    const result = await query(
