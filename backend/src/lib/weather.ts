 ...
 */
export async function fetchAQIData(latitude: number, longitude: number): Promise<AQIData | null> {
  if (!OPENWEATHER_API_KEY) {
    console.warn('OpenWeatherMap API key not configured');
    return null;
  }
 ...
 */
export async function fetchWeatherAndAQI(
  latitude: number,
  longitude: number
): Promise<WeatherAQIData | null> {
 ...
 */
export function needsWeatherUpdate(lastUpdate: Date | null): boolean {
