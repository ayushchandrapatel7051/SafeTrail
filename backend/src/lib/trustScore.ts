...
export function calculateTrustScore(factors: TrustScoreFactors): number {
  ...
  const accountAgeBonus = Math.min(
    accountAgeMonths * ACCOUNT_AGE_BONUS_PER_MONTH,
...
export async function updateUserTrustScore(userId: number): Promise<number> {
  try {
    ...
    const userResult = await query(
      'SELECT EXTRACT(DAY FROM NOW() - created_at)::int as account_age_days FROM users WHERE id = $1',
      [userId]
    );
...
export function getTrustWeight(trustScore: number): number {
  if (trustScore >= 80) return 2.0; // High trust: double weight
  if (trustScore >= 60) return 1.5; // Medium-high trust: 1.5x weight
  if (trustScore >= 40) return 1.0; // Normal trust: normal weight
  if (trustScore >= 20) return 0.7; // Low trust: reduced weight
 ...
 */
export async function calculateWeightedSafetyScore(placeId: number): Promise<number> {
  try {
    const result = await query(
      `SELECT 
       ...
       WHERE r.place_id = $1 AND r.status = 'verified'`,
      [placeId]
    );
