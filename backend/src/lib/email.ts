...
This is an automated emergency alert from SafeTrail.
    ...
    const info = await transporter.sendMail(mailOptions);
    console.log(
      `✅ Emergency email sent successfully to ${options.to}. Message ID: ${info.messageId}`
    );
    return info;
  } catch (error) {
    console.error(`❌ Error sending emergency email to ${options.to}:`, error);
    throw new Error(
      `Failed to send emergency email: ${error instanceof Error ? error.message : String(error)}`
    );
  }
};
