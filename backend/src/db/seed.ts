  ...
  try {
    ...
    for (const report of reports) {
      await query(
        ...
        [userId, placeIds[report.placeIdx], report.type, report.description, report.lat, report.lng, report.status]
      );
    }

    // Create alerts
    await query(
      'INSERT INTO alerts (title, body, severity, created_by) VALUES ($1, $2, $3, $4)',
      ['Festival Crowd Alert', 'Large crowds expected in Chandni Chowk area. Stay vigilant.', 2, adminId]
    );

    await query(
      'INSERT INTO alerts (title, body, severity, created_by) VALUES ($1, $2, $3, $4)',
      ['Weather Warning', 'Heavy fog expected. Avoid night travel in isolated areas.', 1, adminId]
    );
