...
async function updateSafetyScores() {
  ...
  try {
    ...
    for (const [cityId, cityPlaces] of citiesMap) {
      ...
      console.log(
        `📍 ${cityName} (${cityPlaces.length} places | Avg reports: ${Math.round(cityAvg)})`
      );
      ...
      for (const place of cityPlaces) {
        ...
        console.log(
          `  ${place.name.padEnd(33)} | ${String(reportCount).padStart(3)} reports | Score: ${String(safetyScore).padStart(4)} ${contextTags.join('')}`
        );
        updated++;
      }
      console.log('');
    ...
    stats.rows.forEach((row) => {
      console.log(
        `   ${row.category.padEnd(25)} | ${String(row.count).padStart(2)} places | Avg: ${row.avg_reports} reports`
      );
    });
  } catch (error) {
