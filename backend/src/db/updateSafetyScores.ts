...
/**
 * Context-aware safety score calculation
 * 
 * Key insights:
 * - Transit hubs (stations, airports) have higher reports due to traffic, not danger
 * - Tourist areas have more reports due to visibility, but are often safer
 * - Residential areas with low reports may still have risks
 * - Real safety is relative within city context
 * 
 * Realistic scoring (no place is perfectly safe or completely unsafe):
 * - 75-85: Very safe areas (low crime, good infrastructure)
 * - 60-74: Safe areas (moderate crime, decent safety)
 * - 45-59: Caution needed (higher crime, stay alert)
 * - 30-44: Unsafe areas (high crime, avoid if possible)
 * - 20-29: Very unsafe (extreme crime, serious risk)
 */
...
function getPlaceContext(name: string): PlaceContext {
  const nameLower = name.toLowerCase();
  
  return {
    isTransitHub: nameLower.includes('station') || nameLower.includes('terminus') || 
                  nameLower.includes('airport') || nameLower.includes('chowk'),
    isTouristSpot: nameLower.includes('gate') || nameLower.includes('fort') || 
                    nameLower.includes('palace') || nameLower.includes('mahal') ||
                    nameLower.includes('temple') || nameLower.includes('beach') ||
                    nameLower.includes('garden') || nameLower.includes('mantar'),
    isResidential: nameLower.includes('nagar') || nameLower.includes('puram') ||
                   nameLower.includes('colony') || nameLower.includes('vihar')
  };
}
...
async function updateSafetyScores() {
  ...
  try {
    ...
    const citiesMap = new Map<number, typeof places>();
    ...
    for (const [cityId, cityPlaces] of citiesMap) {
      ...
      for (const place of cityPlaces) {
        ...
        await client.query(
          'UPDATE places SET safety_score = $1 WHERE id = $2',
          [safetyScore, place.id]
        );
        
        const contextTags = [];
        if (context.isTransitHub) contextTags.push('🚉');
        if (context.isTouristSpot) contextTags.push('🎭');
        if (context.isResidential) contextTags.push('🏘️');
